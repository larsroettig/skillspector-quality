...
from skillspector_quality.quality.scorers import SkillDoc
...
def quality_scorer(state: QualityState) -> dict[str, object]:
    ...
    report = score_quality(file_cache, config)
    ...
    # Cost is a separate axis, not part of the quality number: over 207 real skills the two
    # correlate at only +0.14, so blending them would hide information (ADR-0009). No LLM is
    # involved — cost is pure arithmetic over the file_cache.
    cost = score_cost(SkillDoc.from_file_cache(file_cache))

    return {"quality_report": report.to_dict(), "cost_report": cost.to_dict()}
